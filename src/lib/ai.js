 ...
 */
export async function sendChatMessage({ messages, fen, apiKey, model = "gpt-4o-mini" }) {
    if (!apiKey) {
...
export async function explainPosition({ fen, moveHistory, apiKey, model }) {
    const moveStr = moveHistory.length > 0 ? moveHistory.join(" ") : "No moves yet";
 ...
 */
export async function getHint({ fen, moveHistory, apiKey, model, hintLevel = 1 }) {
    const levels = {
 ...
 */
export async function evaluateMove({ fen, lastMove, moveHistory, apiKey, model }) {
    return sendChatMessage({
        messages: [
            {
                role: "user",
