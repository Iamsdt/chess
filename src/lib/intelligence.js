...
const QUALITY_LEVELS = [
    { max: 15, label: "Brilliant", emoji: "💎", color: "cyan", blurb: "top-engine  level" },
    { max: 30, label: "Excellent", emoji: "✨", color: "emerald", blurb: "very strong move" },
    { max: 70, label: "Good", emoji: "👍", color: "green", blurb: "solid choice" },
    { max: 150, label: "Inaccuracy", emoji: "⚠️", color: "yellow", blurb: "minor imprecision" },
    { max: 300, label: "Mistake", emoji: "❌", color: "orange", blurb: "significant error" },
    { max: Infinity, label: "Blunder", emoji: "💥", color: "red", blurb: "serious error" },
];
 ...
 */
export function buildMyMoveCard(preFen, moveSan, preResult, postResult, userElo = 1000, msgSeed = 0) {
    ...
    const scoreBefore = scoreFromPerspective(
        preResult?.scoreCp, preResult?.isMate, preResult?.mateIn, preTurn, playerColor
    );
    ...
    const scoreAfterRaw = scoreFromPerspective(
        postResult?.scoreCp, postResult?.isMate, postResult?.mateIn, postFenTurn, playerColor
    );
    ...
    const quality = cpLost !== null ? classifyMove(Math.max(0, cpLost)) : QUALITY_LEVELS[2]; // default Good
    ...
    const evalAfterRaw = postResult?.isMate
        ...
        : postResult?.scoreCp !== null
            ? (() => {
                const fromWhite = postFenTurn === "w" ? postResult.scoreCp : -postResult.scoreCp;
    ...
    const evalAfter = postResult?.isMate
        ...
        : evalAfterRaw !== null
            ? (evalAfterRaw > 0 ? `+${evalAfterRaw.toFixed(2)}` : `${evalAfterRaw.toFixed(2)}`)
            : null;

    // Alternative suggestion (only when move wasn't best)
    let suggestion = null;
    if (!playedIsBest && bestSan && (quality.label === "Inaccuracy" || quality.label === "Mistake" || quality.label === "Blunder")) {
        suggestion = {
            bestMove: bestSan,
        ...
        for (let c = 0; c < 8; c++) {
            ...
            if (isSquareAttackedBy(game, square, attacker)) {
                ...
                if (!isSquareAttackedBy(game, square, victimColor)) {
                    hanging.push({ square, piece: sq.type, value: PIECE_VALUES[sq.type] || 0 });
                }
            }
        }
    }
