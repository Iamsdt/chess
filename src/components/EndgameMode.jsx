      ...
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border shrink-0">
          <div>
            ...
            <h2 className="text-base font-semibold text-foreground mt-0.5">Pick a position to study</h2>
        ...
        <div className="overflow-y-auto flex-1 py-1">
          {filtered.map((s) => {
            ...
            return (
              ...
              >
                <span className="text-xl mt-0.5 shrink-0">{GOAL_ICON[s.goal] ?? "♟"}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span className="text-sm font-semibold text-foreground">{s.title}</span>
                    <span className={`text-[10px] font-semibold uppercase tracking-wide border rounded px-1.5 py-0.5 ${diff.color} ${diff.bg}`}>
                      {s.difficulty}
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground line-clamp-2">{s.description}</p>
                  <p className="text-[10px] text-primary/70 mt-1 font-medium">Goal: {s.goalText}</p>
                </div>
          ...
          {filtered.length === 0 && (
            <p className="text-center text-muted-foreground text-sm py-10">No scenarios in this category</p>
          )}
        </div>
        ...
        <div className="px-4 py-2.5 border-t border-border bg-secondary/20 shrink-0">
          <p className="text-[10px] text-muted-foreground text-center">
            {filtered.length} scenario{filtered.length !== 1 ? "s" : ""} · Click any to load it onto the main board
          </p>
        </div>
      </div>
    </div>
